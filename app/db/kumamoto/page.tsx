...
import db from "@/data/gym-db/kumamoto.json";

export const metadata: Metadata = {
  title: "熊本のパーソナルジム全データベース【60店・実在確認済み】評点と口コミ数で一覧比較",
  description:
    "熊本エリアのパーソナルジム60店を、Googleマップの実測データ(評点・口コミ件数・所在地・2026年8月23日取得)で一覧化した全店データベース。提携の有無に関係なく実在確認できた店舗を網羅し、口コミ件数順に並べています。",
};
...
const faqs = [
  {
    ...
    a: "Googleマップ(Places API)で「パーソナルジム 熊本」を検索し、営業中と確認できた店舗の名称・評点・口コミ件数・所在地を2026年8月23日に機械取得したものです。当サイトとの提携有無に関係なく掲載しており、評点・件数は取得時点のGoogleマップ表示値をそのまま転記しています(当サイトによる評価ではありません)。",
  },
  {
...
export default function KumamotoDbPage() {
  const gyms = db.gyms as Gym[];
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqLd) }} />
      <h1 className="text-2xl font-bold sm:text-3xl">熊本のパーソナルジム全データベース({gyms.length}店)</h1>
      <p className="mt-3 text-sm leading-relaxed text-gray-600">
        熊本エリアで実在確認できたパーソナルジム{gyms.length}店の一覧です(Googleマップ実測・{db.fetchedAt}取得・口コミ件数順)。

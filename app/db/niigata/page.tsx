...
import db from "@/data/gym-db/niigata.json";
...
export const metadata: Metadata = {
  title: "新潟のパーソナルジム全データベース【60店・実在確認済み】評点と口コミ数で一覧比較",
  description:
    "新潟市エリアのパーソナルジム60店を、Googleマップの実測データ(評点・口コミ件数・所在地・2026年8月29日取得)で一覧化した全店データベース。提携の有無に関係なく実在確認できた店舗を網羅し、口コミ件数順に並べています。",
};
...
const faqs = [
  {
    ...
    a: "Googleマップ(Places API)で「パーソナルジム 新潟市」を検索し、営業中と確認できた店舗の名称・評点・口コミ件数・所在地を2026年8月29日に機械取得したものです。当サイトとの提携有無に関係なく掲載しており、評点・件数は取得時点のGoogleマップ表示値をそのまま転記しています(当サイトによる評価ではありません)。",
  },
  {
...
export default function NiigataDbPage() {
  const gyms = db.gyms as Gym[];
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqLd) }} />
      <h1 className="text-2xl font-bold sm:text-3xl">新潟のパーソナルジム全データベース({gyms.length}店)</h1>
      <p className="mt-3 text-sm leading-relaxed text-gray-600">
        新潟市エリアで実在確認できたパーソナルジム{gyms.length}店の一覧です(Googleマップ実測・{db.fetchedAt}取得・口コミ件数順)。
        おすすめを絞った記事は<Link href="/area/niigata/" className="underline text-blue-700">新潟のパーソナルジムおすすめ(編集部の絞り込み)</Link>へ。
      ...
      <p className="mt-8 text-sm">
        <Link href="/area/niigata/" className="underline text-blue-700">→ 新潟のおすすめ記事(編集部の絞り込み)</Link>
